...
import me.skorrloregaming.InventoryManager;
...
public class SkyBlockMenu {
	...
	public SkyBlockMenu(uSkyBlock plugin, ChallengeLogic challengeLogic) {
		...
		this.inventoryManager = (InventoryManager) plugin.getServer().getPluginManager().getPlugin("InventoryManager");
	}
	...
	public Map.Entry<Inventory, String> displayPartyPlayerGUI(final Player player, final String pname) {
		...
		return new AbstractMap.SimpleEntry(menu, title);
	}
	...
	public Map.Entry<Inventory, String> displayPartyGUI(final Player player) {
		List<String> lores = new ArrayList<>();
		...
		return new AbstractMap.SimpleEntry(menu, "\u00a79" + tr("Island Group Members"));
	}

	public Map.Entry<Inventory, String> displayLogGUI(final Player player) {
		List<String> lores = new ArrayList<>();
		...
		lores.clear();
		return new AbstractMap.SimpleEntry(menu, "\u00a79" + tr("Island Log"));
	}

	public Map.Entry<Inventory, String> displayBiomeGUI(final Player player) {
		List<String> lores = new ArrayList<>();
		...
		return new AbstractMap.SimpleEntry(menu, "\u00a79" + tr("Island Biome"));
	}
	...
	public Map.Entry<Inventory, String> displayChallengeGUI(final Player player, int page, String playerName) {
		...
		return new AbstractMap.SimpleEntry(menu, "\u00a79" + pre("{0} ({1}/{2})", tr("Challenge Menu"), page, total));
	}

	public Map.Entry<Inventory, String> displayIslandGUI(final Player player) {
	...
	private Map.Entry<Inventory, String> createInitMenu(Player player) {
		...
		return new AbstractMap.SimpleEntry(menu, "\u00a79" + tr("Island Create Menu"));
	}
	...
	private Map.Entry<Inventory, String> createMainMenu(Player player) {
		...
		return new AbstractMap.SimpleEntry(menu, "\u00a79" + tr("Island Menu"));
	}
	...
	public void onClick(InventoryClickEvent event) {
		...
		String inventoryName = stripFormatting(inventoryManager.getInventoryName(p));
	...
	private void onClickRestartMenu(final InventoryClickEvent event, final Player p, ItemMeta meta, int slotIndex, ItemStack currentItem) {
		...
		if (slotIndex == 0) {
			p.closeInventory();
			Map.Entry<Inventory, String> menu = createMainMenu(p);
			inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
	...
	private void onClickChallengeMenu(InventoryClickEvent event, ItemStack currentItem, Player p, String inventoryName) {
		...
		if (slotIndex >= CHALLENGE_PAGESIZE && slotIndex < CHALLENGE_PAGESIZE + COLS_PER_ROW
			...
			// Pagination
			p.closeInventory();
			Map.Entry<Inventory, String> menu = displayChallengeGUI(p, currentItem.getAmount(), playerName);
			inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
			return;
		}
		...
		if ((slotIndex % 9) > 0) { // 0,9... are the rank-headers...
			...
			Map.Entry<Inventory, String> menu = displayChallengeGUI(p, page, playerName);
			inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
		} else {
			p.closeInventory();
			if (slotIndex < (CHALLENGE_PAGESIZE / 2)) { // Upper half
				if (page > 1) {
					Map.Entry<Inventory, String> menu = displayChallengeGUI(p, page - 1, playerName);
					inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
				} else {
					p.performCommand("island");
				}
	...
	private void onClickPermissionMenu(InventoryClickEvent event, ItemStack currentItem, Player p, String inventoryName, int slotIndex) {
		...
		if (!plugin.getIslandInfo(p).isLeader(p)) {
			p.closeInventory();
			Map.Entry<Inventory, String> menu = displayPartyGUI(p);
			inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
		}
		...
		for (PartyPermissionMenuItem item : permissionMenuItems) {
			if (currentItem.getType() == item.getIcon().getType()) {
				...
				Map.Entry<Inventory, String> menu = displayPartyPlayerGUI(p, pname);
				inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
				return;
			}
		}
	...
	private void onClickPartyMenu(InventoryClickEvent event, ItemStack currentItem, Player p, ItemMeta meta, SkullMeta skull, int slotIndex) {
		...
		if (meta == null || currentItem.getType() == Material.SIGN) {
			...
			p.closeInventory();
			Map.Entry<Inventory, String> menu = displayPartyPlayerGUI(p, skull.getOwner());
			inventoryManager.createInventory(p, menu.getKey(), menu.getValue());
		}
	}
