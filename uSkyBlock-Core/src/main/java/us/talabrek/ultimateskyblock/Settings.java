...
public class Settings {
	...
	public static boolean general_allowLocationSelection;
	public static boolean general_enableSkyMap;
	...
	public static boolean loadPluginConfig(FileConfiguration config) {
        ...
        // Check skymap. If upgrading and doesn't exist, leave disabled (original behavior)
        general_allowLocationSelection = config.getBoolean("options.general.enable-island-picker", false);
        if (!config.contains("options.general.enable-island-picker")){
            config.set("options.general.enable-island-picker", false);
            changed = true;
        }
        general_enableSkyMap = config.getBoolean("options.general.enable-island-map", false);
        if (!config.contains("options.general.enable-island-map")){
            config.set("options.general.enable-island-map", false);
            changed = true;
        }
		...
		island_chestItems = ItemStackUtil
				.createItemArray(ItemStackUtil.createItemList(config.getStringList("options.island.chestItems")));

		island_schematicName = config.getString("options.island.schematicName");
		if (island_schematicName == null || "yourschematicname".equals(island_schematicName)
				|| "uSkyBlockDefault".equals(island_schematicName)) {
