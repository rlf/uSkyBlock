...
public class DevCommand implements CommandExecutor {
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] split) {
        ...
        if (split.length == 0) {
            ...
            } else if (split[0].equals("protectall") && (sender.hasPermission("usb.mod.protectall"))) {
                sender.sendMessage(ChatColor.YELLOW + "This command is only available using WorldGuard.");
                if (Settings.island_protectWithWorldGuard) {
                    sender.sendMessage(ChatColor.YELLOW + "This command has been disabled.");
                } else {
